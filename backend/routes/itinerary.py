...
def _extract_hotels(spots: List[Dict], destination: Optional[str], region: Optional[str] = None, budget_level: str = "medium", num_days: int = 1, num_people: int = 1) -> List[str]:
    """Extract hotels based on budget tier, using vector search first, then fallback methods"""
    hotels: List[str] = []
    
    # Determine hotel tier from budget level
    tier_map = {"low": "low", "medium": "mid", "high": "high"}
    hotel_tier = tier_map.get(budget_level, "mid")
...
def _simple_itinerary_from_spots(req: ItineraryRequest, spots: List[Dict]) -> ItineraryResponse:
    ...
    # Calculate cost dynamically based on budget tier
    ...
    # Use user-selected hotel if provided (from budget planner), otherwise extract from data
    if req.selected_hotel_name:
        recommended_hotels = [req.selected_hotel_name]
    else:
        recommended_hotels = _extract_hotels(spots, req.destination_city, req.region, req.budget_level or "medium", req.num_days, req.num_of_people or 1)
    ...
    # Prepare transport costs — use user-selected transport if provided
    if req.selected_transport_type:
        enhanced_transport_costs = {
            "selected_transport": req.selected_transport_type,
            "note": f"You chose: {req.selected_transport_type}",
        }
    else:
...
def _build_query_and_fetch_spots(req: ItineraryRequest) -> List[Dict[str, Any]]:
    preferences_dict = req.model_dump()

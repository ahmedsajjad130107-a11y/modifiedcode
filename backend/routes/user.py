...
# Secret key for JWT token encoding — set SECRET_KEY in your .env file
...
async def get_profile(email: str = Depends(verify_token)):
    ...
    if supabase:
        try:
            ...
            if result.data:
                user = result.data[0]
                return UserProfile(email=user["email"], full_name=user.get("full_name"))
        except Exception:
            pass  # Supabase failed — fall through to local auth
...
async def update_profile(
    ...
    # --- Local SQLite fallback ---
    # local_auth doesn't support full_name updates; return current profile data
    try:
        user = get_user_by_email(email)
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
